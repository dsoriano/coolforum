...
if($_REQUEST['action'] == "confirmedit")
{
	...
	{		
		...
		$query					=	$sql->query("UPDATE "._PRE_."config SET valeur = '$Tabl_to_save' WHERE options = 'grades'");
...
if($_REQUEST['action'] == "usegrade")
{
	if($_REQUEST['use_grades'] == "Y")
		$query 					= $sql->query("UPDATE "._PRE_."config SET valeur='Y' WHERE options = 'use_grades'");
	else
		$query 					= $sql->query("UPDATE "._PRE_."config SET valeur='N' WHERE options = 'use_grades'");
